...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 elseif(isset($_POST['resend'])){
  ...
  else{$code=otp_code();$stmt=$con->prepare("INSERT INTO otp_codes(user_id,code,purpose,expires_at) VALUES(?,?,'registration',DATE_ADD(NOW(),INTERVAL 10 MINUTE))");$stmt->bind_param('is',$uid,$code);$stmt->execute();$u=$con->query("SELECT name,email FROM users WHERE id=$uid")->fetch_assoc();try{require_once dirname(__DIR__).'/config/mailer.php';$mail=configured_mailer();$mail->addAddress($u['email'],$u['name']);$mail->Subject='Your new CareerSim code';$mail->Body='<p>Your new code is <strong>'.e($code).'</strong>.</p>';$mail->send();flash('success','A new code was sent.');}catch(Throwable $e){$error='The code was created but email delivery failed. You can retry resend in 60 seconds.';}}
 }else{
