...
$types = '';
if ($q !== '') {
    $where[] = '(MATCH(c.title,c.summary,c.description) AGAINST(? IN NATURAL LANGUAGE MODE) OR c.title LIKE ?)';
    $params[] = $q;
    $params[] = '%' . $q . '%';
    $types .= 'ss';
}
...
$categories = $con->query('SELECT * FROM career_categories ORDER BY name');
$pageTitle = 'Explore careers';
$bodyClass = 'student-catalog';
require dirname(__DIR__) . '/includes/header.php'; ?>
<div class="career-catalog-shell">
    <header class="career-catalog-header">
        <h1>Explore Careers</h1>
        <p>Find your path, then experience the work.</p>
    </header>
    <form class="career-filter-card">
        ...
        <select class="form-select" name="category">
            <option value="0">All categories</option><?php foreach ($categories as $c): ?><option value="<?= $c['id'] ?>" <?= $category === $c['id'] ? 'selected' : '' ?>><?= e($c['name']) ?></option><?php endforeach ?>
        </select>
        ...
        <?php foreach ($careers as $c): $growth = $c['growth_outlook'] ?: 'stable'; ?>
            <article class="catalog-career-card">
                <div class="catalog-career-media">
                    <?php if ($c['image']): ?><img src="<?= e(preg_match('~^https?://~i', $c['image']) ? $c['image'] : url($c['image'])) ?>" alt="<?= e($c['title']) ?> workspace"><?php else: ?><div class="catalog-career-placeholder"><i class="bi bi-briefcase"></i></div><?php endif ?>
                    <div>
                        <h2><?= e($c['title']) ?></h2>
                    </div>
                </div>
                <div class="catalog-career-body">
                    ...
                    <div class="catalog-meta mt-auto">
                        ...
                        <div><span>Average salary</span><strong><?= $c['average_salary'] ? 'R' . number_format($c['average_salary']) : 'Not listed' ?></strong></div>
                    </div>
